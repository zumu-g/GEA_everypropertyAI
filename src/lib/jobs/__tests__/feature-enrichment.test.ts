...
// Mock the two live source modules so the enricher is exercised without network.
...
describe('buildFeatureRow (pure)', () => {
  it('maps planning + nearest train station (happy path)', () => {
    const row = buildFeatureRow('14-loders-way-berwick-vic-3806', PLANNING, TRANSPORT, ISO);
...
describe('enrichFeaturesForPoint (fail-soft per source)', () => {
  ...
  it('a thrown planning lookup does not block the transport result (R3)', async () => {
  ...
  it('a thrown transport lookup does not block the planning result (R3)', async () => {
